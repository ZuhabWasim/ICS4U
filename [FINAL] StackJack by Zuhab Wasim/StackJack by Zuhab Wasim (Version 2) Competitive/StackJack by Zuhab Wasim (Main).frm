...
Private Sub mnuNewGame_Click()
    ...
    ResetForm lblCount(), picStack(), MAX_STACKS, lblPoints, cmdDiscard, lblTime
...
Private Sub picStack_Click(Index As Integer)
    ...
    ElseIf StackValue(Index) = 21 Then
        ...
        StackCount(Index) = -1
        picStack(Index).Cls
        UpdatePoints lblPoints, Points, StackPoints(POINTS_CLEAR, LastStackClear, CardCounter, ScoreMultiplier)
...
Private Sub tmrTimer_Timer()
    ...
    'Timer goes in seconds
    TimeCount = TimeCount + 0.1
    
    'Outputs the time as M:SS, or HH:MM:SS
    lblTime.Caption = ConvTime(TimeCount)
    
    'If the user reaches 59:59:59 for their time, they lose
    If TimeCount = 216000 - 1 Then
        GameOverMessage = "You lose! You have run out of time! "
        Points = 0
        EndGame Points, TimeCount, cmdDiscard, picStack(), MAX_STACKS, mnuNewGame, tmrTimer
    End If
